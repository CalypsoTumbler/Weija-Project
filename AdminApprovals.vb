...
Public Class AdminApprovals
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Else
            ...
            Try
                Con.Open()
                Dim query As String = "SELECT * FROM Approval WHERE ApprovedBy = @Nametxt AND ApprovalCode = @hashString"
                ...
                If a <> 0 Then
                    MsgBox("User already saved", vbExclamation, "Error")
                    Clear()
                    Con.Close()
                Else
                    query = "INSERT INTO Approval(ApprovedBy, ApprovalCode, PhoneNumber) VALUES(@Nametxt, @hashString, @Phonetxt)"
                    ...
                    MsgBox("Approver Added", vbInformation, "Success")
                    Con.Close()
                    DisplayUsers()
                    Clear()
                End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            ...
            Try
                Con.Open()
                Dim query = "Update Approval set ApprovedBy = '" & Nametxt.Text & "', ApprovalCode = '" & hashString & "', PhoneNumber = '" & Phonetxt.Text & "' where Id = " & Key & ""
                Dim cmd As SqlCommand
                cmd = New SqlCommand(query, Con)
                cmd.ExecuteNonQuery()
                MsgBox("Approver Information Edited", vbInformation, "Success")
                Con.Close()
                DisplayUsers()
                Clear()
            Catch ex As Exception
    ...
    Private Sub UsersDGV_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles UsersDGV.CellContentClick
        Try
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            ...
            Else
                Con.Open()
                Dim query = "delete from Approval where Id = " & Key & ""
                Dim cmd As SqlCommand
                cmd = New SqlCommand(query, Con)
                cmd.ExecuteNonQuery()
                MsgBox("Approver Deleted", vbInformation, "Success")
                Con.Close()
                DisplayUsers()
                Clear()
            End If
